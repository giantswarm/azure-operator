...
//
// service/controller/v10/resource/<resource>/template/<template>
//
...
func ARMTemplateURI(version, resource, template string) string {
	return fmt.Sprintf("https://raw.githubusercontent.com/giantswarm/azure-operator/%s/service/controller/v10/resource/%s/template/%s", version, resource, template)
}
