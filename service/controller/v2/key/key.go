...
func AdminUsername(customObject providerv2alpha1.AzureConfig) string {
...
func AdminSSHKeyData(customObject providerv2alpha1.AzureConfig) string {
	users := customObject.Spec.Cluster.Kubernetes.SSH.UserList
	// We don't want panics when someone is doing something nasty.
	if len(users) == 0 {
		return ""
...
func ClusterCustomer(customObject providerv2alpha1.AzureConfig) string {
	return customObject.Spec.Cluster.Customer.ID
}

// ClusterID returns the unique ID for this cluster.
func ClusterID(customObject providerv2alpha1.AzureConfig) string {
	return customObject.Spec.Cluster.ID
}

// ClusterNamespace returns the cluster Namespace for this cluster.
func ClusterNamespace(customObject providerv2alpha1.AzureConfig) string {
	return customObject.Spec.Cluster.ID
}
...
func ClusterOrganization(customObject providerv2alpha1.AzureConfig) string {
	return ClusterCustomer(customObject)
}

// ClusterTags returns a map with the resource tags for this cluster.
func ClusterTags(customObject providerv2alpha1.AzureConfig, installationName string) map[string]string {
...
func DNSZoneAPI(customObject providerv2alpha1.AzureConfig) string {
...
// parent DNS zone.
func DNSZoneResourceGroupEtcd(customObject providerv2alpha1.AzureConfig) string {
...
// parent DNS zone.
func DNSZoneResourceGroupIngress(customObject providerv2alpha1.AzureConfig) string {
	return customObject.Spec.Azure.DNSZones.Ingress.ResourceGroup
}

func IsDeleted(customObject providerv2alpha1.AzureConfig) bool {
...
func MasterSecurityGroupName(customObject providerv2alpha1.AzureConfig) string {
...
func WorkerSecurityGroupName(customObject providerv2alpha1.AzureConfig) string {
...
func MasterSubnetName(customObject providerv2alpha1.AzureConfig) string {
...
func WorkerSubnetName(customObject providerv2alpha1.AzureConfig) string {
	return fmt.Sprintf("%s-%s-%s", ClusterID(customObject), virtualNetworkSuffix, workerSubnetSuffix)
}

func MasterVMSSName(customObject providerv2alpha1.AzureConfig) string {
	return fmt.Sprintf("%s-master", ClusterID(customObject))
}

func WorkerVMSSName(customObject providerv2alpha1.AzureConfig) string {
	return fmt.Sprintf("%s-worker", ClusterID(customObject))
}

// MasterNICName returns name of the master NIC.
func MasterNICName(customObject providerv2alpha1.AzureConfig) string {
...
func ResourceGroupName(customObject providerv2alpha1.AzureConfig) string {
	return ClusterID(customObject)
}

// RouteTableName returns name of the route table for this cluster.
func RouteTableName(customObject providerv2alpha1.AzureConfig) string {
	return fmt.Sprintf("%s-%s", ClusterID(customObject), routeTableSuffix)
}

func ToCustomObject(v interface{}) (providerv2alpha1.AzureConfig, error) {
	if v == nil {
		return providerv2alpha1.AzureConfig{}, microerror.Maskf(wrongTypeError, "expected '%T', got '%T'", &providerv1alpha1.AzureConfig{}, v)
	}

	customObjectPointer, ok := v.(*providerv2alpha1.AzureConfig)
	if !ok {
		return providerv2alpha1.AzureConfig{}, microerror.Maskf(wrongTypeError, "expected '%T', got '%T'", &providerv1alpha1.AzureConfig{}, v)
	}
...
func VersionBundleVersion(customObject providerv2alpha1.AzureConfig) string {
	return customObject.Spec.VersionBundle.Version
}

// VnetName returns name of the virtual network.
func VnetName(customObject providerv2alpha1.AzureConfig) string {
	return fmt.Sprintf("%s-%s", ClusterID(customObject), virtualNetworkSuffix)
}

func VnetCIDR(customObject providerv2alpha1.AzureConfig) string {
	return customObject.Spec.Azure.VirtualNetwork.CIDR
}

func VnetCalicoSubnetCIDR(customObject providerv2alpha1.AzureConfig) string {
	return customObject.Spec.Azure.VirtualNetwork.CalicoSubnetCIDR
}

func VnetMasterSubnetCIDR(customObject providerv2alpha1.AzureConfig) string {
	return customObject.Spec.Azure.VirtualNetwork.MasterSubnetCIDR
}

func VnetWorkerSubnetCIDR(customObject providerv2alpha1.AzureConfig) string {
	return customObject.Spec.Azure.VirtualNetwork.WorkerSubnetCIDR
}

func VNetID(customObject providerv2alpha1.AzureConfig, subscriptionID string) string {
