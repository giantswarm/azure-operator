...
func (r *Resource) GetCurrentState(ctx context.Context, obj interface{}) (interface{}, error) {
	...
	if key.IsDeleted(cr) {
		r.logger.LogCtx(ctx, "level", "debug", "message", "redirecting deletion to namespace termination") // nolint: errcheck
	...
	// again on the next resync period.
	{
		...
		if IsNetworkInterfacesNotFound(err) {
			r.logger.LogCtx(ctx, "level", "debug", "message", "did not find the network interfaces in the Azure API") // nolint: errcheck
			resourcecanceledcontext.SetCanceled(ctx)
			r.logger.LogCtx(ctx, "level", "debug", "message", "canceling resource") // nolint: errcheck
	...
	{
		r.logger.LogCtx(ctx, "level", "debug", "message", "looking for the master endpoints in the Kubernetes API") // nolint: errcheck

		n := key.ClusterNamespace(cr)
		manifest, err := r.k8sClient.CoreV1().Endpoints(n).Get(masterEndpointsName, apismetav1.GetOptions{})
		if apierrors.IsNotFound(err) {
			r.logger.LogCtx(ctx, "level", "debug", "message", "did not find the master endpoints in the Kubernetes API") // nolint: errcheck

			return nil, nil
		} else if err != nil {
			return nil, microerror.Mask(err)
		} else {
			r.logger.LogCtx(ctx, "level", "debug", "message", "found the master endpoints in the Kubernetes API") // nolint: errcheck
			endpoints = manifest
		}
	}
