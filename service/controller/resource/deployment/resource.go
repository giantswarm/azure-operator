...
func (r *Resource) enrichControllerContext(ctx context.Context, customObject providerv1alpha1.AzureConfig) error {
	...
	{
		v, err := r.getDeploymentOutputValue(ctx, customObject, "legacy_masters_subnet", "masterSubnetID")
		if IsNotFound(err) {
			// fall through
		} else if err != nil {
			return microerror.Mask(err)
	...
	{
		v, err := r.getDeploymentOutputValue(ctx, customObject, "legacy_workers_subnet", "workerSubnetID")
		if IsNotFound(err) {
			// fall through
		} else if err != nil {
			return microerror.Mask(err)
