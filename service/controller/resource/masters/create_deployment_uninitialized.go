...
func (r *Resource) deploymentUninitializedTransition(ctx context.Context, obj interface{}, currentState state.State) (state.State, error) {
	...
	if controllercontext.IsInvalidContext(err) {
		...
		res, err := organizationAzureClientSet.DeploymentsClient.CreateOrUpdate(ctx, key.ClusterID(&cr), key.MastersVmssDeploymentName, computedDeployment)
		if err != nil {
			return currentState, microerror.Mask(err)
		}
		...
		r.InstanceWatchdog.DeleteFailedVMSS(ctx, organizationAzureClientSet.VirtualMachineScaleSetVMsClient, key.ResourceGroupName(cr), key.MasterVMSSName(cr))
