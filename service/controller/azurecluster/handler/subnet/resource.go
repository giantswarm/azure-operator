...
func (r *Resource) EnsureCreated(ctx context.Context, obj interface{}) error {
	...
	natGatewaysClient, err := r.azureClientsFactory.GetNatGatewaysClient(ctx, azureCluster)
	if err != nil {
		return microerror.Mask(err)
	}

	storageAccountsClient, err := r.azureClientsFactory.GetStorageAccountsClient(ctx, azureCluster)
	if err != nil {
		return microerror.Mask(err)
	}

	subnetsClient, err := r.azureClientsFactory.GetSubnetsClient(ctx, azureCluster)
	if err != nil {
		return microerror.Mask(err)
	}
