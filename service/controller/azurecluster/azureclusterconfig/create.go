...
func (r *Resource) buildAzureClusterConfig(ctx context.Context, cluster capiv1alpha3.Cluster, azureCluster capzv1alpha3.AzureCluster, masters, workers []capzv1alpha3.AzureMachine) (corev1alpha1.AzureClusterConfig, error) {
	...
	{
		releaseVersion := cluster.GetLabels()[label.ReleaseVersion]
		if !strings.HasPrefix(releaseVersion, "v") {
			releaseVersion = fmt.Sprintf("v%s", releaseVersion)
		}

		r.logger.LogCtx(ctx, "level", "debug", "message", fmt.Sprintf("fetching Release using release version %#q in cr", releaseVersion))
		err = r.ctrlClient.Get(ctx, client.ObjectKey{Namespace: corev1.NamespaceAll, Name: releaseVersion}, &release)
		if err != nil {
			return corev1alpha1.AzureClusterConfig{}, microerror.Mask(err)
		}
		r.logger.LogCtx(ctx, "level", "debug", "message", "fetched release object")
	}

	clusterOperatorVersion, err := key.ComponentVersion(release, "cluster-operator")
	if err != nil {
		return corev1alpha1.AzureClusterConfig{}, microerror.Mask(err)
	}
		...
		Spec: corev1alpha1.AzureClusterConfigSpec{
			Guest: corev1alpha1.AzureClusterConfigSpecGuest{
				ClusterGuestConfig: corev1alpha1.ClusterGuestConfig{
					...
					VersionBundles:    componentsToClusterGuestConfigVersionBundles(release.Spec.Components),
				},
