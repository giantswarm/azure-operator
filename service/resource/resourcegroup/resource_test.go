...
func Test_Resource_ResourceGroup_GetDesiredState(t *testing.T) {
	...
	var newResource *Resource
	{
		...
		for i, tc := range testCases {
			...
			group := result.(*azureresource.Group)
...
func Test_Resource_ResourceGroup_GetCreateState(t *testing.T) {
	testCases := []struct {
		...
		Cur                   interface{}
		Des                   interface{}
		ExpectedResourceGroup *azureresource.Group
	}{
		{
			...
			Cur: &azureresource.Group{
				Name: to.StringPtr("5xchu"),
			},
			Des: &azureresource.Group{
				Name: to.StringPtr("5xchu"),
			},
	...
	for i, tc := range testCases {
		...
		if tc.ExpectedResourceGroup == nil {
			if tc.ExpectedResourceGroup != result {
				t.Fatalf("case %d expected '%#v' got '%#v'", i+1, tc.ExpectedResourceGroup, result)
			}
		} else {
			name := *result.(*azureresource.Group).Name
			expectedName := *tc.ExpectedResourceGroup.Name

			if expectedName != name {
				t.Fatalf("case %d expected '%s' got '%s'", i+1, expectedName, name)
			}
		}
	}
}
...
func Test_Resource_ResourceGroup_GetUpdateState(t *testing.T) {
	...
	for i, tc := range testCases {
		...
		if createState != nil {
			t.Fatalf("case %d expected nil create state got '%#v'", i+1, createState)
		}
		if deleteState != nil {
			t.Fatalf("case %d expected nil delete state got '%#v'", i+1, createState)
		}
		if updateState != nil {
			t.Fatalf("case %d expected nil update state got '%#v'", i+1, createState)
		}
	}
}
