...
func CreateGSReleaseContainingOperatorVersion(ctx context.Context, config Config) (*releasev1alpha1.Release, error) {
	err := ensureReleaseCRD(ctx, config)
	if err != nil {
...
func CreateGSReleaseContainingOperatorVersionWithIngressAndHelloWorld(ctx context.Context, config Config) (*releasev1alpha1.Release, error) {
	err := ensureReleaseCRD(ctx, config)
	if err != nil {
		return &releasev1alpha1.Release{}, microerror.Mask(err)
	}
	...
	var release *releasev1alpha1.Release
	{
		config.Logger.LogCtx(ctx, "level", "debug", "message", "ensuring Release exists", "release", ReleaseName)
		release = buildReleaseCR()
		nginx := releasev1alpha1.ReleaseSpecApp{
			ComponentVersion: "1.9.2",
			Name:             "nginx-ingress-controller",
			Version:          "1.9.2",
		}
		helloworld := releasev1alpha1.ReleaseSpecApp{
			ComponentVersion: "0.0.1",
			Name:             "hello-world",
			Version:          "0.0.1",
		}
		release.Spec.Apps = append(release.Spec.Apps, nginx, helloworld)
		_, err := config.K8sClients.G8sClient().ReleaseV1alpha1().Releases().Create(ctx, release, metav1.CreateOptions{})
		if err != nil {
			return &releasev1alpha1.Release{}, microerror.Mask(err)
		}
...
func ensureReleaseCRD(ctx context.Context, config Config) error {
...
func buildReleaseCR() *releasev1alpha1.Release {
