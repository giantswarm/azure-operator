...
func (s *MultiAZ) Test(ctx context.Context) error {
	...
	if len(zones) != 1 {
		return microerror.Maskf(wrongAvailabilityZones, "The amount of AZ's used is not correct. Expected %d zones, got %d zones", 1, len(zones))
	}
	if zones[0] != "1" {
		return microerror.Maskf(wrongAvailabilityZones, "The AZ used is not correct. Expected %s, got %s", "1", zones[0])
	}
